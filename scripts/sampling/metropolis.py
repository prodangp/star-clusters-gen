...
sampler = MetropolisSampler(num_samples=3000, num_dimensions=7, random_seed=42, verbose=True)
step_size = 0.5
sampler.set_bounds(clusters.get_ave_bounds())
samples = sampler.sample(pdf, step_size=step_size, random_walk=False, markov_chain=False, burn_in_max=500)
np.save(f'{model_path}/rwMC_{timestamp}_{step_size}_cluster.npy', samples)
